...
def drawPiece(sol,color):
	...
	for i in a.data:
		if ori==0:
			rectlist.append(w.create_rectangle(50+(-xlow+x+mult*i[1])*scale,50+(y+i[0])*scale,50+(-xlow+1+x+mult*i[1])*scale,50+(1+y+i[0])*scale,fill=color, outline=color))
		if ori==1:
			rectlist.append(w.create_rectangle(50+(-xlow+x+i[0])*scale,50+(y-mult*i[1])*scale,50+(-xlow+1+x+i[0])*scale,50+(1+y-mult*i[1])*scale,fill=color, outline=color))
		if ori==2:
			rectlist.append(w.create_rectangle(50+(-xlow+x-mult*i[1])*scale,50+(y-i[0])*scale,50+(-xlow+1+x-mult*i[1])*scale,50+(1+y-i[0])*scale,fill=color, outline=color))
		if ori==3:
			rectlist.append(w.create_rectangle(50+(-xlow+x-i[0])*scale,50+(y+mult*i[1])*scale,50+(-xlow+1+x-i[0])*scale,50+(1+y+mult*i[1])*scale,fill=color, outline=color))
